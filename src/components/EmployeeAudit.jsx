...
export default function EmployeeAudit() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <h2 className="text-3xl font-extrabold text-slate-900">Personal Audit History</h2>
        <p className="text-slate-500 mt-1">Immutable change log details of edits, updates, and approvals on your goal sheets.</p>
      </div>

      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center gap-2">
          <History className="w-5 h-5 text-teal-600"/>
          <h3 className="text-lg font-bold text-slate-800">My Change Ledger</h3>
        </div>
          ...
          <table className="w-full text-sm text-left text-slate-600">
            <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4">Modified Element</th>
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {myAuditLogs.length === 0 && (
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-slate-500 font-medium">No historical transactions logged for this employee profile yet.</td>
                </tr>
              )}
            </tbody>
          </table>
